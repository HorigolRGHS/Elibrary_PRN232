...
export const CategoryService = {
  ...
  getById: async (id: number): Promise<CategoryReadDTO> => {
    try {
      console.debug(
        "[CategoryService] trying GET",
        `/catalog/api/categories/(${id})`
      );
      const raw = await api.get<CategoryReadDTO>(
        `/catalog/api/categories/(${id})`
      );
      const item = _extractItemById(raw, id);
      if (item) {
        console.debug(
          "[CategoryService] success GET",
          `/catalog/api/categories/(${id})`,
          item
        );
        return item;
    ...
    try {
      ...
      console.error(
        "[CategoryService] fallback list GET failed",
        listErr?.message
      );
    }
  ...
  getAdminItem: async (id: number): Promise<CategoryReadDTO> => {
    ...
    try {
      console.debug(
        "[CategoryService] fallback: GET list to locate admin item",
        id
      );
      const list = await api.get<CategoryReadDTO[]>(`/catalog/api/categories`);
      const found = _extractItemById(list, id);
    ...
    const e: any = new Error(
      "Admin Category GET failed: single-item admin endpoint not available (tried admin and fallback forms)"
    );
...
const _getCandidateId = (it: any) =>
  it?.CategoryId ?? it?.categoryId ?? it?.id ?? undefined;
...
const _extractItemById = (raw: any, id?: number) => {
    ...
    if ("data" in item && item.data !== undefined) item = item.data;
      ...
      if (id !== undefined)
        return item.value.find(
          (el: any) => Number(_getCandidateId(el)) === Number(id)
        );
  ...
  if (Array.isArray(item)) {
    if (id !== undefined)
      return item.find((el: any) => Number(_getCandidateId(el)) === Number(id));
    return item[0];
  }
  return item;
};
